...
class StockAnalyzer:
    ...
    # Train new model
        ...
        try:
            X = np.asarray(all_features, dtype=np.complex128)
            y = np.asarray(all_targets, dtype=np.complex128)
            ...
            y = np.asarray([label_map[label] for label in y], dtype=np.complex128)
