...
def run(
    ...
    if _has_active_position(symbol):
        msg = (
    ...
    # ── 7. Eksekusi Order — non-blocking ─────────────────────────
    # notify diteruskan ke executor untuk update saat entry fill & SL/TP terpasang
    ...
    order_msg = (
        f"{side_emoji} <b>ENTRY ORDER SENT — {symbol}</b>\n"
        f"─────────────────────────\n"
