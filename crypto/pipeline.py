"""
pipeline.py — Orchestrator utama algo trading.

Flow lengkap untuk satu symbol:
  1. Train XGBoost (1000 candle, min 500)
  2. Backtest before vs after
  3. Predict 3 candle ke depan + confidence
  4. Jika confidence < 55% atau NEUTRAL → skip
  5. Kirim ke DeepSeek R1
  6. Jika AI → BUYING / SELLING → eksekusi limit order
  7. Return notif Telegram

Dipanggil dari telegram_bot.py via /analyze BTCUSDT
atau dari scheduler untuk run otomatis.
"""
...
def run(
    ...
    Args:
        ...
        interval : timeframe (default '30m')
        notify   : callable(str) untuk kirim pesan ke Telegram secara progresif.
                   Dipanggil dengan HTML string di tiap tahap penting.

    Returns:
        dict ringkasan hasil pipeline:
        {
          "symbol", "interval",
          "stage":   tahap terakhir yang dicapai,
          "skipped": bool,
          "skip_reason": str,
          "train":   train_result,
          "backtest": bt_result,
          ...
          "ai":      ai_result,
          "order":   order_result,
          "messages": list[str],   ← semua pesan Telegram yang dikirim
        }
    """
    def _notify(msg: str):
        if notify:
    ...
    if not train_result["ok"]:
        ...
        result.update({"stage": "train_failed", "skipped": True,
                        "skip_reason": train_result["reason"],
                        "messages": [msg]})
        return result
