...
def check_circuit_breaker() -> Tuple[bool, float]:
    ...
    Paper: modal = PAPER_BALANCE_USDT
    Live : modal = wallet balance dari Binance (cache 35s)
    """
    from config import PAPER_TRADING_MODE, PAPER_BALANCE_USDT
    ...
    # Tentukan basis modal
    if PAPER_TRADING_MODE:
        modal_basis = PAPER_BALANCE_USDT
    else:
        try:
            from order.executor import get_available_balance as _live_bal
            modal_basis = _live_bal()
            if modal_basis <= 0:
                modal_basis = PAPER_BALANCE_USDT  # fallback
        except Exception:
            modal_basis = PAPER_BALANCE_USDT

    today_pnl = _get_today_pnl()
    loss_pct  = (-today_pnl) / modal_basis  # positif = rugi
...
def get_floating_drawdown() -> Tuple[float, float]:
    ...
    Paper: basis modal = PAPER_BALANCE_USDT
    Live : basis modal = wallet balance Binance
    """
    import requests
    from config import PAPER_TRADING_MODE, PAPER_BALANCE_USDT
...
def get_risk_summary_text() -> str:
    ...
    if cb_left > 0:
        ...
        from config import PAPER_TRADING_MODE, PAPER_BALANCE_USDT
        if PAPER_TRADING_MODE:
            _modal = PAPER_BALANCE_USDT
        else:
            try:
                from order.executor import get_available_balance as _lb
                _modal = _lb() or PAPER_BALANCE_USDT
            except Exception:
                _modal = PAPER_BALANCE_USDT
