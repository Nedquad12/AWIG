"""
risk_manager.py — Centralized risk management untuk paper trading.

Fitur:
  1. SL Ban      : simbol yang kena SL di-ban 4 sesi ke depan
  2. Circuit Breaker : kalau rugi >= 5% modal hari ini → stop 8 sesi (posisi berjalan dibiarkan)
  3. WTI Filter  : max 3 posisi dengan WTI >= 65%, tolak yang baru jika sudah 3
  4. Daily Stats : simpan history PnL harian ke /home/ec2-user/crypto/history/
  5. Daily Drawdown notif
"""
...
_STATE_FILE     = "/home/ec2-user/crypto/ban/risk_state.json"   # ban list, CB state
_PAPER_POS_FILE = "/home/ec2-user/crypto/paper_positions.json"
_PAPER_HIST_FILE= "/home/ec2-user/crypto/paper_history.json"

os.makedirs(_HISTORY_DIR, exist_ok=True)

# ── Constants ───────────────────────────────────────────────────────────────
SL_BAN_SESSIONS       = 4      # sesi ban setelah kena SL
CIRCUIT_BREAKER_PCT   = 0.05   # 5% daily loss → circuit breaker
CIRCUIT_BREAKER_SESS  = 8      # berapa sesi CB aktif
WTI_CORR_THRESHOLD    = 65.0   # WTI % dianggap "highly correlated"
MAX_CORR_POSITIONS    = 4      # max posisi dengan WTI >= threshold
...
def check_circuit_breaker() -> Tuple[bool, float]:
    ...
    # CB sudah aktif — cek sisa sesi
    if state.get("cb_sessions_left", 0) > 0:
        return True, 0.0

    # Hitung PnL hari ini dari history
    today_pnl = _get_today_pnl()
    loss_pct  = (-today_pnl) / PAPER_BALANCE_USDT  # positif = rugi
...
def check_wti_slot(wti_pct: float) -> Tuple[bool, str]:
    """
    Cek apakah boleh masuk posisi baru dengan WTI tertentu.
...
def save_daily_stats() -> Optional[str]:
    """
    Simpan ringkasan trading hari ini ke /home/ec2-user/crypto/history/YYYY-MM-DD.json.
    Dipanggil di akhir sesi atau saat CB triggered.
    Return path file yang disimpan, atau None jika gagal.
    """
    from config import PAPER_BALANCE_USDT
    ...
    try:
        ...
        stats = {
            ...
            "pnl_pct":        round(total / PAPER_BALANCE_USDT * 100, 2),
            "wins":           len(wins),
            "losses":         len(losses),
...
def get_daily_summary_text() -> str:
    ...
    # Fallback: hitung langsung dari history
    if s is None:
        pnl     = _get_today_pnl()
        pnl_pct = pnl / PAPER_BALANCE_USDT * 100
        return (
...
def get_floating_drawdown() -> Tuple[float, float]:
    ...
    Membutuhkan mark price dari Binance — jika gagal return 0.
    """
    import requests
    from config import PAPER_BALANCE_USDT
    ...
    try:
        ...
        dd_pct = (-total_float) / PAPER_BALANCE_USDT if total_float < 0 else 0.0
...
def get_risk_summary_text() -> str:
    ...
    if cb_left > 0:
        ...
        today_pnl  = _get_today_pnl()
        from config import PAPER_BALANCE_USDT
        loss_pct   = max(-today_pnl / PAPER_BALANCE_USDT * 100, 0)
