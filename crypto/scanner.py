...
#
# FIX: BTC Multiplier logic diubah dari multiplicative/divisive
# ke ADDITIVE adjustment. Alasan:
#
# Masalah lama:
#   altcoin = +3.0, BTC mult = +2.0 → +3.0 × 2.0 = +6.0
#   altcoin = +6.0, BTC mult = -2.0 → +6.0 / 2.0 = +3.0
#   Threshold 5.0 tidak punya makna konsisten karena multiply dan
#   divide menghasilkan range yang sangat berbeda dan tidak simetris.
#
# Fix: additive approach
#   BTC_CONTEXT_BONUS = f(abs(btc_total)) → nilai 0 sampai +2.0
#   Jika arah sama → tambahkan bonus
#   Jika arah berlawanan → kurangi bonus
#   Threshold tetap punya makna konsisten
# =============================================================
...
# ------------------------------------------------------------------
# BTC Multiplier — ADDITIVE (bukan multiplicative)
# ------------------------------------------------------------------

def _btc_context_bonus(abs_weighted_total: float) -> float:
    """
    Konversi abs weighted total BTC ke bonus additive.
    Nilainya kecil (max 2.0) agar tidak mendominasi signal altcoin.
...
def get_btc_context(interval: str = DEFAULT_INTERVAL) -> tuple[float, float]:
    """
    Scan BTCUSDT untuk market context.
    Return (btc_weighted_total, signed_bonus).

    signed_bonus:
      BTC bullish → +bonus (ditambahkan ke altcoin bullish, dikurangi dari bearish)
      BTC bearish → -bonus (dikurangi dari altcoin bullish, ditambahkan ke bearish)
      BTC netral  → 0.0
    """
...
def apply_btc_context(weighted_total: float, btc_bonus: float) -> float:
    """
    Apply BTC context bonus secara additive dan simetris.

    Arah sama dengan BTC  → tambah bonus (dikuatkan)
    Arah lawan BTC        → kurangi bonus (dilemahkan, tidak sampai flip)

    Contoh (BTC bonus = +1.0):
      altcoin = +3.0 → +3.0 + 1.0 = +4.0  (sama arah, dikuat)
      altcoin = -2.0 → -2.0 - 1.0 = -3.0  (lawan arah, dilemah)

    Contoh (BTC bonus = -1.0):
      altcoin = +3.0 → +3.0 - 1.0 = +2.0  (lawan BTC, dilemah)
      altcoin = -2.0 → -2.0 + 1.0 = -1.0  (sama arah BTC bearish, dikuat)

    Hasilnya: threshold tetap punya makna konsisten karena hanya
    bergeser ±bonus, tidak dikalikan.
    """
    if btc_bonus == 0.0:
        return weighted_total

    # Arah sama → bonus menguatkan
    if (btc_bonus > 0 and weighted_total > 0) or \
       (btc_bonus < 0 and weighted_total < 0):
        return round(weighted_total + btc_bonus, 4)

    # Arah berlawanan → bonus melemahkan
...
# Candle minimum yang dibutuhkan trainer untuk ATR-based labeling
# Harus konsisten dengan MIN_CANDLE_TRAIN di config
_MIN_CANDLES_FOR_PIPELINE = 100

def _get_listed_symbols() -> set[str]:
    """
    Ambil set simbol yang sudah listed cukup lama dari exchangeInfo.
    Filter: onboardDate < 30 hari lalu → buang.
    Ini mencegah koin baru masuk pipeline dan gagal di trainer.
    """
    ...
    try:
        ...
        return set()  # kalau gagal, jangan filter (fail open)


def get_top_symbols(top_n: int = SCAN_TOP_N) -> list[str]:
    # Ambil simbol yang sudah cukup lama listed
    ...
    usdt_perps = [
        ...
        # Filter koin baru: skip kalau exchangeInfo berhasil tapi simbol tidak ada di old_enough
...
def score_symbol(
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Guard: butuh minimal _MIN_CANDLES_FOR_PIPELINE candle agar
            # trainer punya cukup data untuk ATR-based labeling.
            # Koin dengan candle < threshold ini pasti gagal di trainer — skip sekarang.
            # Catatan: trainer fetch ulang sendiri (limit=1000), tapi jumlah candle
            # yang tersedia di Binance sama. Kalau scanner dapat < threshold,
            # trainer juga akan dapat < threshold.
...
def scan(
    ...
    """
    Flow:
      1. Scan BTCUSDT → dapat BTC additive bonus
      2. Scan top_n altcoin secara paralel
      3. Apply BTC additive bonus ke weighted_total setiap altcoin
    ...
    logger.info(
        "[scanner] Done. BTC=%.4f (bonus=%+.2f) | %d/%d scored | %d lolos | %d gagal.",
        btc_total, btc_bonus, len(results), len(symbols), len(passed), failed,
    )
    return passed
