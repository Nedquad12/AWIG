...
def execute_paper_order(ai_result: dict, pred: dict, notify_fn=None) -> dict:
    ...
    position = {
        "paper_id":          paper_id,
        "session_id":        ai_result.get("session_id", ""),  # untuk tracking cancel sesi
        "symbol":            symbol,
        "side":              side,
...
def execute_paper_market_order(ai_result: dict, pred: dict, notify_fn=None) -> dict:
    """
    Eksekusi paper MARKET ORDER langsung (status="open" seketika).
    Digunakan untuk "special coin" yang memenuhi 2x persyaratan board.

    Berbeda dengan execute_paper_order():
      - Status langsung "open" (tidak perlu tunggu harga hit entry)
      - Fill price = mark price saat ini (bukan entry price dari board)
      - Tidak masuk ke sistem pending / monitor fill

    Return dict sama seperti execute_paper_order() untuk kompatibilitas.
    """
    ...
    symbol      = pred["symbol"]
    action      = ai_result["action"]           # "BUYING" | "SELLING"
    stop_loss   = float(ai_result["stop_loss"])
    take_profit = float(ai_result["take_profit"])
    leverage    = int(ai_result["leverage"])
    qty_fraction = float(ai_result.get("qty_fraction", RISK_PER_TRADE_PCT / 100))
    ...
    # ── Ambil mark price sekarang ─────────────────────────────────────────────
    try:
        resp       = _requests.get(
            f"{BINANCE_DATA_URL}/fapi/v1/premiumIndex",
        ...
        logger.error("[paper:market] Gagal ambil mark price %s: %s", symbol, e)
        return {"ok": False, "symbol": symbol, "reason_fail": f"Gagal ambil mark price: {e}"}

    if mark_price <= 0:
        return {"ok": False, "symbol": symbol, "reason_fail": "Mark price invalid (0)"}

    # ── Hitung qty dari balance ───────────────────────────────────────────────
    balance      = _load_balance()
    margin_usdt  = round(balance * qty_fraction, 4)
    notional     = round(margin_usdt * leverage, 4)

    # Cek re-entry penalty (sama seperti limit order)
    history      = _load_history()
    last_tp      = next(
        (h for h in reversed(history)
         if h.get("symbol") == symbol and h.get("close_reason", "").startswith("TP")),
        None,
    )
    reentry_applied = False
    if last_tp:
        margin_usdt  = round(margin_usdt  * (1 - REENTRY_MARGIN_CUT), 4)
        notional     = round(margin_usdt  * leverage, 4)
        reentry_applied = True

    # Hitung qty (pakai step size minimal; paper tidak perlu rounding ketat)
    qty = round(notional / mark_price, 6) if mark_price > 0 else 0

    # ── Partial TP price ──────────────────────────────────────────────────────
    if side == "BUY":
        partial_tp_price = round(mark_price + (take_profit - mark_price) * PARTIAL_TP_RR / (take_profit / mark_price - 1 + 1e-9), 8)
        # Sederhanakan: partial TP di RR 1.5 dari entry ke TP
        sl_dist          = mark_price - stop_loss
        tp_dist          = take_profit - mark_price
        partial_tp_price = round(mark_price + tp_dist * (PARTIAL_TP_RR / (tp_dist / sl_dist + 1e-9)), 8) if sl_dist > 0 else take_profit
    else:
        sl_dist          = stop_loss - mark_price
        tp_dist          = mark_price - take_profit
        partial_tp_price = round(mark_price - tp_dist * (PARTIAL_TP_RR / (tp_dist / sl_dist + 1e-9)), 8) if sl_dist > 0 else take_profit
    ...
    position = {
        ...
        "symbol":           symbol,
        "side":             side,
        "status":           "open",           # langsung open
        "order_type":       "MARKET",         # penanda ini market order
        "entry_price":      mark_price,       # fill di mark price saat ini
        ...
        "balance_used":     margin_usdt,
        "qty_fraction":     qty_fraction,
        "wti_pct":          float(ai_result.get("wti_pct", 0.0)),
        "reentry_applied":  reentry_applied,
        "created_at":       now_ts,
        "filled_at":        now_ts,
        "special_coin":     True,             # penanda special coin
    }
    ...
    note_parts = []
    if reentry_applied:
        note_parts.append(f"Re-entry penalty -{REENTRY_MARGIN_CUT*100:.0f}% margin")
    note_parts.append("⭐ Special coin — market order langsung")
    note = " | ".join(note_parts)

    side_emoji = "🟢" if side == "BUY" else "🔴"
    _notify(
        f"{side_emoji} <b>PAPER MARKET ORDER — {symbol}</b>\n"
        f"─────────────────────────\n"
        f"  ID          : <code>{paper_id}</code>\n"
        f"  Side        : <b>{side}</b>\n"
        f"  Fill @ Mark : <code>{mark_price}</code>\n"
        f"  Stop Loss   : <code>{stop_loss}</code>\n"
        f"  Take Profit : <code>{take_profit}</code>\n"
        f"  TP 30%      : <code>{partial_tp_price:.8f}</code>\n"
        f"  Leverage    : <b>{leverage}x</b>\n"
        f"  Qty         : <code>{qty}</code>\n"
        f"  Margin      : <code>{margin_usdt} USDT</code>\n"
        f"  ⭐ <i>{note}</i>"
    )
    logger.info(
        "[paper:market] MARKET FILLED %s %s @ %.6f qty=%.6f margin=%.2f special=True",
        paper_id, symbol, mark_price, qty, margin_usdt,
    )
...
def cancel_session_pending(session_id: str, notify_fn=None) -> list:
    """
    Cancel semua posisi pending dari session_id tertentu.
    Dipanggil monitor saat filled >= MAX_FILLS_PER_SESSION (2) dari sesi yang sama.
    Return list paper_id yang di-cancel.
    """
    ...
    try:
        ...
        for i, pos in enumerate(positions):
            if (
                pos.get("status") == "pending"
                and pos.get("session_id") == session_id
            ):
                positions[i]["status"]        = "cancelled"
                positions[i]["cancel_reason"] = "sesi sudah 2 filled — slot dibebaskan"
                positions[i]["cancelled_at"]  = int(time.time())
                changed = True
                ...
                _notify(
                    f"⚡ <b>Auto-Cancel — {sym}</b>\n"
                    f"  ID     : <code>{pid}</code>\n"
                    f"  Reason : Sesi sudah 2 order filled\n"
                    f"  Status : ❌ Cancelled"
                )
                logger.info(
                    "[paper] Auto-cancel pending %s %s — sesi %s sudah 2 filled",
                    pid, sym, session_id,
                )
...
def count_session_filled(session_id: str) -> int:
    """Hitung berapa posisi dari session_id ini yang sudah filled (open)."""
    if not session_id:
        return 0
    try:
        positions = _load_positions()
