    ...
    try:
        ...
        for i, pos in enumerate(positions):
            if pos.get("status") == "pending" and pos.get("session_id") == session_id:
                ...
                # Live: cancel semua open orders Binance (entry + SL + TP)
                if not PAPER_TRADING_MODE and pos.get("live"):
                    try:
                        from order.executor import cancel_all_open_orders
                        cancel_all_open_orders(sym)
                        logger.info("[paper] cancel_session_pending: all Binance orders cancelled %s", sym)
                    except Exception as e:
                        logger.warning("[paper] Gagal cancel Binance orders %s: %s", sym, e)
        if changed:
            _save_positions(positions)
    except Exception as e:
...
def execute_paper_order(ai_result: dict, pred: dict, notify_fn=None) -> dict:
    ...
    if is_live:
        try:
            ...
            return {"ok": False, "symbol": symbol,
                    "reason_fail": f"Bracket order gagal: {e}"}

        position["binance_order_id"] = bracket["entry_order_id"]
        position["sl_order_id"]      = bracket["sl_order_id"]
        position["tp_order_id"]      = bracket["tp_order_id"]
        position["leverage"]         = leverage

        positions = _load_positions()
        positions.append(position)
        _save_positions(positions)
        # SL/TP sudah aktif di Binance sejak awal.
        # Timeout 2 jam dihandle monitor.cancel_expired_pending.
    ...
    return {
        ...
        "qty_fraction": round(qty_fraction, 6),
        "wti_pct":      wti_pct,
        "note": f"{'LIVE BRACKET — SL/TP aktif di Binance sejak awal ✅' if is_live else 'PAPER LIMIT — entry dipantau via WS'}",
    }
