"""
order/executor.py — Eksekusi limit order ke Binance USDⓈ-M Futures (Testnet).

Flow:
  1. Set leverage untuk symbol
  2. Hitung quantity dari available balance × RISK_PER_TRADE_PCT / entry_price
  3. Place limit order (BUY atau SELL)
  4. Return dict hasil order untuk notif Telegram

Pakai demo-fapi.binance.com (testnet).
"""
...
# ------------------------------------------------------------------
# Helper: ambil lot size & price precision dari exchange info
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# Helper: ambil available balance USDT
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# Public: execute_order
# ------------------------------------------------------------------

def execute_order(ai_result: dict, pred: dict) -> dict:
    """
    Eksekusi limit order berdasarkan keputusan AI.

    Args:
        ai_result : dict dari ai/analyst.py (action, entry_price, stop_loss,
                    take_profit, leverage, reason)
        pred      : dict dari ml/predictor.py (symbol, interval, dll)

    Returns:
        {
          "ok": bool,
          "reason_fail": str (jika gagal),
          "symbol": str,
          "side": "BUY" | "SELL",
          "order_id": int,
          "qty": float,
          ...
          "take_profit": float,
          "leverage": int,
          "balance_used": float,   ← USDT yang dialokasikan
        }
    """
    symbol      = pred["symbol"]
