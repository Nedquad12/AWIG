...
def _post_batch(path: str, params: dict) -> list:
    """POST ke batchOrders — batchOrders dikirim sebagai URL-encoded param."""
    params["timestamp"]  = int(time.time() * 1000)
    params["recvWindow"] = RECV_WINDOW
    qs  = urllib.parse.urlencode(params)
...
def get_position_risk(symbol: str | None = None) -> list:
    """Ambil posisi risk dari Binance (/fapi/v2/positionRisk)."""
    params = {}
    if symbol:
...
def get_open_positions() -> list:
    """Ambil semua posisi yang sedang terbuka (positionAmt != 0)."""
...
def place_bracket_order(
    ...
    """
    Kirim LIMIT entry + SL + TP sekaligus via /fapi/v1/batchOrders.
    SL/TP pakai closePosition=true — Binance hold sebagai conditional,
    aktif otomatis begitu posisi terbuka (sama seperti web/app Binance).

    Return dict: entry_order_id, sl_order_id, tp_order_id (int | None).
    ...
    resp_list = _post_batch("/fapi/v1/batchOrders", {"batchOrders": _json.dumps(batch)})

    entry_resp = resp_list[0] if len(resp_list) > 0 else {}
    sl_resp    = resp_list[1] if len(resp_list) > 1 else {}
    tp_resp    = resp_list[2] if len(resp_list) > 2 else {}
    ...
    sl_id    = sl_resp.get("orderId") if not sl_resp.get("code") else None
    tp_id    = tp_resp.get("orderId") if not tp_resp.get("code") else None

    if not sl_id:
        logger.error("[executor] Bracket SL gagal %s: %s", symbol, sl_resp)
    if not tp_id:
        logger.error("[executor] Bracket TP gagal %s: %s", symbol, tp_resp)

    logger.info("[executor] Bracket %s | entry=%s sl=%s tp=%s", symbol, entry_id, sl_id, tp_id)
    return {
