...
Alur:
  1. Set leverage
  2. Validasi qty vs MIN_NOTIONAL, MIN_QTY, MAX_QTY dari exchange info
  3. Place LIMIT entry → return segera ke pipeline (non-blocking)
  4. Background thread poll setiap 3 detik, max 20 menit
  5. Jika FILLED → pasang SL + TP (reduceOnly + quantity eksplisit)
  6. Jika timeout / cancel → log + notify tanpa mengganggu apapun
  7. Jika SL/TP gagal setelah fill → emergency market close
"""
...
# Cap notional maksimum per trade — proteksi token micro-price
...
def _get_symbol_info(symbol: str) -> dict:
    """
    Ambil semua filter penting dari exchange info:
      - LOT_SIZE   : stepSize, minQty, maxQty
      - PRICE_FILTER: tickSize
      - MIN_NOTIONAL: notional minimum per order
    """
...
def _round_step(value: float, step: float) -> float | int:
    """
    Bulatkan qty sesuai stepSize Binance.
    Kalau stepSize >= 1 (token micro-price), return int — Binance tidak terima float.
    Kalau stepSize < 1, return float dengan presisi yang tepat.
    """
    if step >= 1.0:
        # stepSize=1 → qty harus integer bulat, tanpa .0
...
def _round_price(value: float, tick: float) -> float:
    """
    Bulatkan harga sesuai tickSize Binance.
    Pakai Decimal untuk hindari floating point error pada tick kecil (0.00001).
    """
    ...
    while time.time() < deadline:
        ...
        try:
            ...
            if status == "FILLED":
                ...
                # Round ulang pakai sym_info — pastikan integer kalau stepSize >= 1
...
def execute_order(ai_result: dict, pred: dict, notify_fn=None) -> dict:
    ...
    Guards yang dicek sebelum order:
      1. Arah SL/TP valid
      2. qty >= min_qty dan qty <= max_qty
      3. notional (qty × price) >= min_notional dan <= MAX_NOTIONAL_USDT
    """
    symbol       = pred["symbol"]
    ...
    try:
        ...
        # ── 2. Ambil symbol info (semua filter) ───────────────────
        sym_info  = _get_symbol_info(symbol)
        available = _get_available_balance()

        # ── 3. Hitung notional & qty ──────────────────────────────
        # Cap notional ke MAX_NOTIONAL_USDT untuk proteksi token micro-price
        ...
        # ── 7. Background thread untuk poll + SL/TP ──────────────
        t = threading.Thread(
            target=_sltp_worker,
