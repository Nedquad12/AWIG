...
def _topic_for(msg: str) -> int:
    ...
    # General — semua output pipeline lainnya (training, WFV, scan, prediksi, dll)
    pipeline_keywords = [
        "Sesi #", "Scan", "Training ML", "Walk-Forward", "Fold Detail",
        "Regime", "ML Prediction", "WFV", "Pipeline", "token lolos",
        "Cleared", "BTC Context", "⏳", "🌡️", "📊", "🔮",
        "skip WTI", "banned", "di-skip",
    ]
    if any(k in msg for k in pipeline_keywords):
...
async def run_session(bot: Bot) -> None:
    ...
    if not passed:
        await _broadcast(
            bot,
            f"✅ <b>Sesi #{session_id} selesai</b> — tidak ada token lolos.",
            topic_id=TOPIC_GENERAL,
        )
    ...
    for candidate in passed:
        ...
        if banned:
            ...
            await _broadcast(
                bot,
                f"🚫 <b>{symbol}</b> — banned {ban_rem} sesi (kena SL)",
                topic_id=TOPIC_GENERAL,
            )
            continue
        ...
        if not wti_ok:
            ...
            await _broadcast(
                bot,
                f"🔗 <b>{symbol}</b> skip WTI — <i>{wti_reason}</i>",
                topic_id=TOPIC_GENERAL,
            )
            continue
        ...
        try:
            result = await loop.run_in_executor(
                ...
                lambda s=symbol, w=wti_pct: run_pipeline(
                    ...
                    notify=_make_notify(loop, bot),  # auto-detect topic per pesan
                    wti_pct=w,
                ),
            )
