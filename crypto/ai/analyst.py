"""
ai/analyst.py — Kirim konteks lengkap ke DeepSeek R1, parse keputusan trading.

DeepSeek R1 menerima:
  - 1000 candle (OHLCV + MA10/20/50, RSI14, Vol MA10/20, Freq MA10/20)
  - Skor tiap indikator + bobot ML-adjusted
  - Hasil backtest (accuracy, winrate)
  - Prediksi ML (direction, confidence, predicted_price)

Output JSON yang diharapkan dari AI:
  {
    ...
    "take_profit": float,
    "leverage": int,
    "reason": "penjelasan detail kenapa AI mengambil keputusan ini"
  }

Jika action = BUYING atau SELLING → teruskan ke order/executor.py
Jika action = HOLD → skip
"""
...
MAX_CANDLES_FOR_AI = 200   # kirim 200 candle terakhir ke AI (hemat token)
