...
SKIP if ANY of these conditions are true:
  ...
  - Actual winrate (net of cost) is below minimum required
  - ML confidence < 55%
  - Winrate warning present (⚠️) AND actual < minimum
...
def analyze(pred: dict, bt_result: dict, train_result: dict) -> dict:
    ...
    # FIX: pass n_signals untuk validasi winrate di Kelly
    n_signals_long  = bt_after.get("n_signal_up", 0)
    n_signals_short = bt_after.get("n_signal_dn", 0)
