...
async def cmd_buatorder(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    mode_tag = "PAPER" if PAPER_TRADING_MODE else "REAL"
    await update.message.reply_text(
...
async def _bo_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if PAPER_TRADING_MODE:
            from order.paper_executor import execute_paper_order
            ...
            _bal = _live_bal()
            ai_result["qty_fraction"] = min(modal / _bal, 1.0) if _bal > 0 else 0.01
            result = await loop.run_in_executor(
                None,
                lambda: execute_paper_order(ai_result, pred, notify_fn=_notify),
            )
...
async def cmd_portofolio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not PAPER_TRADING_MODE:
        await _cmd_portofolio_live(update, context)
        return

    # ── PAPER mode: dari JSON (implementasi lama) ────────────────────────
    try:
        import time as _time
        from datetime import datetime, timezone, timedelta
        from order.paper_executor import _load_history, get_available_balance
        from config import PAPER_BALANCE_USDT
        ...
        modal_awal = PAPER_BALANCE_USDT
        modal_kini = modal_awal + total_pnl
        ...
        # ── Build pesan ───────────────────────────────────────────────────
        lines = [
            f"💼 <b>Portofolio Paper Trading</b>\n",
...
async def _cmd_portofolio_live(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Portofolio live — semua data dari Binance Futures API."""
    import json, os
    msg = await update.message.reply_text("⏳ Mengambil data dari Binance...")
    loop = asyncio.get_event_loop()
    ...
    def _fetch_positions():
        return bc.get_open_positions()

    def _fetch_sl_tp(symbol):
        try:
            orders = bc.get_open_orders(symbol)
    ...
    try:
        ...
        live_pos = await loop.run_in_executor(None, _fetch_positions)

        wallet   = float(account.get("totalWalletBalance",    0))
        margin_b = float(account.get("totalMarginBalance",    0))
        unrealized = float(account.get("totalUnrealizedProfit", 0))
        available  = float(account.get("availableBalance",    0))
        init_margin = float(account.get("totalInitialMargin", 0))

        unr_str   = f"+{unrealized:.2f}" if unrealized >= 0 else f"{unrealized:.2f}"
        unr_emoji = "📈" if unrealized >= 0 else "📉"

        lines = [
            "💰 <b>Portofolio Live — Binance Futures</b>\n",
            f"  Wallet Balance  : <code>{wallet:,.2f} USDT</code>",
            f"  Margin Balance  : <code>{margin_b:,.2f} USDT</code>",
            f"  Unrealized PnL  : {unr_emoji} <b>{unr_str} USDT</b>",
            f"  Tersedia        : <b>{available:,.2f} USDT</b>",
            f"  Margin terpakai : <code>{init_margin:,.2f} USDT</code>",
        ]

        # Posisi aktif dengan dual SL/TP
        pos_file = os.path.abspath(os.path.join(os.path.dirname(__file__), "paper_positions.json"))
        json_pos_map = {}
        if os.path.exists(pos_file):
            ...
            for p in _all:
                if p.get("status") == "open" and p.get("live"):
                    json_pos_map[p.get("symbol", "")] = p

        if live_pos:
            lines.append(f"\n📊 <b>Posisi Aktif ({len(live_pos)})</b>")
            for p in live_pos:
                sym    = p.get("symbol", "")
                amt    = float(p.get("positionAmt", 0))
                entry  = float(p.get("entryPrice",  0))
                mark   = float(p.get("markPrice",   0))
                unr_p  = float(p.get("unRealizedProfit", 0))
                lev    = p.get("leverage", "?")
                side   = "LONG" if amt > 0 else "SHORT"
                emoji  = "🟢" if amt > 0 else "🔴"
                ps     = f"+{unr_p:.2f}" if unr_p >= 0 else f"{unr_p:.2f}"

                orders     = await loop.run_in_executor(None, _fetch_sl_tp, sym)
                sl_binance = f"<code>{orders['sl']:.6f}</code>" if orders["sl"] else "<i>tidak ada</i>"
                tp_binance = f"<code>{orders['tp']:.6f}</code>" if orders["tp"] else "<i>tidak ada</i>"

                jp = json_pos_map.get(sym, {})
                sl_sistem = tp_sistem = be_note = ""
                if jp:
                    be_hit = jp.get("breakeven_hit", False)
                    sl_val = jp.get("stop_loss")
                    tp_val = jp.get("take_profit")
                    if sl_val:
                        trail = " (trailing)" if be_hit else ""
                        sl_sistem = f"<code>{sl_val}</code>{trail}"
                    if tp_val:
                        tp_sistem = f"<code>{tp_val}</code>"
                    be_note = "  ⚖️ Breakeven aktif\n" if be_hit else ""

                lines.append(
                    f"\n{emoji} <b>{sym}</b> {side} ×{lev}\n"
                    f"  Qty         : <code>{abs(amt)}</code>\n"
                    f"  Entry       : <code>{entry}</code>  Mark: <code>{mark}</code>\n"
                    f"  PnL         : <b>{ps} USDT</b>\n"
                    f"{be_note}"
                    f"  SL sistem   : {sl_sistem or '<i>-</i>'}\n"
                    f"  SL Binance  : {sl_binance}\n"
                    f"  TP sistem   : {tp_sistem or '<i>-</i>'}\n"
                    f"  TP Binance  : {tp_binance}"
                )
        else:
...
async def cmd_monitor(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not _MONITOR_AVAILABLE or _monitor is None:
        ...
        return
    # get_status_text sudah menampilkan dual SL/TP (diupdate di monitor.py)
    # Tambahkan info pending dari JSON
    import json, os, time as _time
    lines = [_monitor.get_status_text()]
    ...
    try:
        if os.path.exists(pos_file):
            with open(pos_file) as _f:
                _all = json.load(_f)
            pending = [p for p in _all if p.get("status") == "pending"]
            if pending:
                lines.append("\n─────────────────────")
                ...
                for p in pending:
                    ...
                    mode_t = " 💰LIVE" if is_live else " 🧪PAPER"
                    emoji  = "🟢" if side == "BUY" else "🔴"
                    opened = float(p.get("opened_at", 0))
                    age_m  = int((_time.time() - opened) / 60) if opened else 0
                    expire = max(0, 120 - age_m)
                    lines.append(
                        f"{emoji} <b>{sym}</b>{mode_t}\n"
                        f"  {side} ×{lev}  entry: <code>{entry}</code>\n"
                        f"  Expire ~{expire} menit\n"
                    )
    except Exception:
        pass
