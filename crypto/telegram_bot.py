...
# States
# ------------------------------------------------------------------
_BO_SYMBOL, _BO_SIDE, _BO_ENTRY, _BO_SL, _BO_TP, _BO_CONFIRM = range(6)

# Leverage dan qty fixed default
_BO_LEVERAGE     = 10
_BO_QTY_FRACTION = 0.01   # 1% dari balance
...
async def _bo_symbol(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✅ Symbol: <b>{symbol}</b>\n\nSide? Pilih <b>BUY</b> atau <b>SELL</b>",
        parse_mode=ParseMode.HTML,
...
async def _bo_tp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    d = context.user_data
    rr = abs(tp - entry) / abs(entry - d["sl"])
    ...
    await update.message.reply_text(
        ...
        f"  Side     : <b>{d['side']}</b>\n"
        f"  Entry    : <code>{d['entry']}</code>\n"
        f"  Stop Loss: <code>{d['sl']}</code>\n"
        f"  Take Prof: <code>{tp}</code>\n"
        f"  RR Ratio : <code>{rr:.2f}</code>\n"
        f"  Leverage : <b>{_BO_LEVERAGE}x</b>\n"
        f"  Qty Frac : <b>{_BO_QTY_FRACTION*100:.0f}% balance</b>\n\n"
...
async def _bo_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Buat fake ai_result dan pred yang compatible dengan execute_order
    ai_result = {
        "action":       action,
        "entry_price":  d["entry"],
        "stop_loss":    d["sl"],
        "take_profit":  d["tp"],
        "leverage":     _BO_LEVERAGE,
        "qty_fraction": _BO_QTY_FRACTION,
    }
    pred = {"symbol": symbol}
    ...
    try:
        ...
        if result.get("ok"):
            ...
            await update.message.reply_text(
                f"❌ Order gagal: <code>{result.get('reason_fail', 'unknown')}</code>",
                parse_mode=ParseMode.HTML,
            )
