...
async def cmd_scan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🔍 Scanning top 100 token, harap tunggu...", parse_mode="HTML")
    ...
    try:
        # Scan tanpa threshold (threshold=0 agar semua masuk)
        ...
        # Kirim per batch 25 token agar tidak melebihi batas pesan Telegram
        batch_size = 25
        for batch_num, i in enumerate(range(0, len(results), batch_size), 1):
            batch = results[i:i+batch_size]
            lines = [f"📊 <b>Scan Result — Batch {batch_num}</b>\n"]
