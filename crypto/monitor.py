...
class PositionMonitor:
    ...
    def __init__(
        ...
        self.vol_analyzer = VolumeAnalyzer(spike_multiplier=15.5)
    ...
    async def _check_pending_fills(self) -> None:
        ...
        Live mode : cek status order ke Binance via get_order_status.
                    Lebih akurat karena fill price dan qty langsung dari exchange.
        Paper mode: cek via mark price lokal (tidak ada Binance order).

        Jika filled >= MAX_FILLS_PER_SESSION dari sesi yang sama → cancel sisa pending.
        ...
        for pos in pending:
            ...
            if is_live and binance_oid:
                ...
                try:
                    ...
                    if status == "FILLED":
                        ...
                        # Order dibatal dari Binance sendiri — sync JSON
                        logger.warning(
                            "[monitor] Live order %s %s — Binance status=%s, cancel di JSON",
                            paper_id, sym, status,
                        )
                        from order.paper_executor import cancel_pending_position
            ...
            if session_id:
                ...
                if total_filled >= MAX_FILLS_PER_SESSION:
                    # Cancel semua sisa pending dari sesi yang sama
