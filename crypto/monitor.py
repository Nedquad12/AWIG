...
class PositionMonitor:
    ...
    async def _check_pending_fills(self) -> None:
        """
        Cek semua pending order apakah sudah hit entry price.
        ...
        for pos in pending:
            ...
            # Cek apakah harga sudah hit entry
            hit = (side == "BUY" and mark <= entry) or (side == "SELL" and mark >= entry)
            if not hit:
                continue

            # Fill posisi ini
            filled = activate_pending_position(paper_id, mark, notify_fn=self.notify)
            if not filled:
                continue

            logger.info("[monitor] Pending filled %s %s @ %.6f", paper_id, sym, mark)
    ...
    async def _evaluate_position(self, pos: MonitoredPosition, price: float) -> None:
        ...
        if not pos.breakeven_hit and pos.is_breakeven_triggered(price):
            ...
            # Live: amend SL Binance ke entry (proteksi ganda)
            if pos.raw.get("live") and not self.paper_mode:
                try:
                    ...
                    if os.path.exists(POSITIONS_FILE):
                        ...
                        _raw_pos = pos.raw
                    loop = asyncio.get_event_loop()
                    _updated = await loop.run_in_executor(
                        None, lambda: amend_breakeven_sl(_raw_pos, notify_fn=self.notify)
                    )
                    if _updated.get("sl_order_id") != _raw_pos.get("sl_order_id"):
                        pos.raw = _updated
                except Exception as _be_err:
                    logger.error("[monitor] amend_breakeven_sl error %s: %s", symbol, _be_err)

            be_live_note = "\n  🔒 SL Binance dipindah ke entry (proteksi ganda)" \
                           if pos.raw.get("live") and not self.paper_mode else ""
            self.notify(
    ...
    async def _close_position(
        ...
        # ── Live mode: close ke Binance ───────────────────────────────────
        is_live = pos.raw.get("live", False)
        if is_live and not self.paper_mode:
            try:
                from order.executor import cancel_all_open_orders, close_position_market
                cancel_all_open_orders(symbol)
                logger.info("[monitor] Cancelled all Binance orders %s sebelum close", symbol)
            except Exception as e:
                logger.warning("[monitor] Gagal cancel Binance orders %s: %s", symbol, e)
            try:
                from order.executor import close_position_market
                close_position_market(symbol, pos.side, pos.qty)
                logger.info("[monitor] Live close sent %s @ %.6f", symbol, close_price)
            except Exception as e:
                logger.error("[monitor] GAGAL close live %s: %s — POSISI TERBUKA!", symbol, e)
                self.notify(
                    f"🚨 <b>{symbol}</b> — GAGAL close live!\n"
                    f"<code>{e}</code>\nCek Binance manual sekarang!",
                    TOPIC_ERROR,
                )
                return  # jangan hapus dari posisi, biar ada jejak
        ...
        self.notify(
            f"{emoji} <b>CLOSED{mode_tag} — {symbol}</b>\n"
            f"  Reason : <i>{reason}</i>\n"
        ...
        logger.info("[monitor] CLOSED%s %s @ %.6f status=%s pnl=%.4f",
                    mode_tag, symbol, close_price, close_status, pnl)
    ...
    def _flush_sl_to_file(self) -> None:
        """Update stop_loss + breakeven state di JSON (preserve sl_order_id live)."""
        updated = []
        for pos in self.positions.values():
            ...
            # Preserve sl_order_id yang mungkin sudah diupdate oleh amend_breakeven_sl
            if "sl_order_id" in pos.raw:
                raw["sl_order_id"] = pos.raw["sl_order_id"]
            updated.append(raw)
        if updated:
    ...
    def get_status_text(self) -> str:
        ...
        for sym, pos in self.positions.items():
            ...
            trail_tag = "(trailing)" if pos.breakeven_hit else ""
            wti_tag  = f"WTI={pos.wti['wti_pct']:.0f}%" if pos.wti else "WTI=?"
            emoji    = "🟢" if pos.side == "BUY" else "🔴"
            is_live  = pos.raw.get("live", False)
            mode_tag = " 💰LIVE" if is_live else " 🧪PAPER"
            part_tag = " 🎯30%✓" if pos.raw.get("partial_tp_done") else ""

            # SL/TP Binance (hanya live, non-blocking)
            sl_binance_line = ""
            tp_binance_line = ""
            if is_live and not self.paper_mode:
                try:
                    ...
                    for o in (orders if isinstance(orders, list) else []):
                        ...
                        if otype == "STOP_MARKET" and sp:
                            be_note = " 🔒BE" if pos.breakeven_hit else ""
                            sl_binance_line = f"\n  SL Binance  : <code>{sp:.6f}</code>{be_note}"
                        elif otype == "TAKE_PROFIT_MARKET" and sp:
                            tp_binance_line = f"\n  TP Binance  : <code>{sp:.6f}</code>"
                except Exception:
                    pass

            lines.append(
                f"{emoji} <b>{sym}</b>{mode_tag}{part_tag}  {wti_tag}\n"
                f"  Entry       : <code>{pos.entry_price}</code>  Mark: <code>{mark or '?'}</code>\n"
                f"  SL sistem   : <code>{pos.sl:.6f}</code> {be_tag}{trail_tag}"
                f"{sl_binance_line}\n"
                f"  TP sistem   : <code>{pos.tp}</code>"
                f"{tp_binance_line}\n"
                f"  PnL         : <b>{pnl_str} USDT</b>\n"
            )
        return "\n".join(lines)
