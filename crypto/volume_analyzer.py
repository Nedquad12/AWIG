"""
volume_analyzer.py — Deteksi volume spike berbasis ATR historis.

Pendekatan baru:
- Saat init_symbol(), fetch aggTrades 1 hari terakhir dari Binance REST API
  dan hitung ATR buy_vol dan sell_vol per-menit dari data tersebut.
- Saat live (via WebSocket), akumulasi volume menit berjalan.
- Spike terjadi jika volume menit berjalan >= SPIKE_MULTIPLIER × ATR historis.

Keuntungan vs pendekatan lama:
- Baseline langsung tersedia sejak detik pertama (tidak perlu "warmup" 14+ menit).
- ATR dihitung dari 1440 data poin (1 hari × 1 menit), jauh lebih representatif
  dari sebelumnya yang hanya 14 bucket.
- Threshold adaptif per simbol (token liquid vs illiquid beda baseline-nya).
"""
...
# Spike trigger
SPIKE_MULTIPLIER  = 15.0   # >= 15x ATR → close paksa
MIN_ATR_BUCKETS   = 30     # minimum bucket historis agar ATR valid (30 menit)
...
class VolumeAnalyzer:
    ...
    def check_sell_spike(self, symbol: str) -> Tuple[bool, str]:
        """
        Cek apakah sell volume menit ini spike >= SPIKE_MULTIPLIER × ATR sell historis.
        Dipakai untuk deteksi reversal pada posisi LONG.
        """
        state = self._states.get(symbol.upper())
    ...
    def check_buy_spike(self, symbol: str) -> Tuple[bool, str]:
        """
        Cek apakah buy volume menit ini spike >= SPIKE_MULTIPLIER × ATR buy historis.
        Dipakai untuk deteksi reversal pada posisi SHORT.
        """
        state = self._states.get(symbol.upper())
        if state is None or not state.baseline_ok:
            return False, ""
