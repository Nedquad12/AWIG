...
class StockAnalyzer:
    ...
    # Train new model
        ...
        try:
            X = np.copy(np.asarray(all_features))
            y = np.copy(np.asarray(all_targets))
            ...
            y = np.copy(np.asarray([label_map[label] for label in y]))
    ...
    def predict_signals(self, data, stock_code):
        ...
        try:
            ...
            # Predict
            X = np.copy(np.asarray([feature_vector]))
