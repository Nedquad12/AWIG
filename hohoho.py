...
class StockAnalyzer:
    ...
    def __init__(self, data_folder="Saham"):
        self.LABEL_MAP = {'SELL': 0, 'HOLD': 1, 'BUY': 2}  # untuk training
        self.REVERSE_MAP = {0: 'SELL', 1: 'HOLD', 2: 'BUY'}
    ...
    def analyze_momentum(self, data):
        ...
        recent_foreign = (data['Foreign_Buy'] -
                          data['Foreign_Sell']).tail(5).mean()
        previous_foreign = (data['Foreign_Buy'] -
                            data['Foreign_Sell']).iloc[-10:-5].mean()
        ...
        ma_slope = (ma7.iloc[-1] - ma7.iloc[-5]) / ma7.iloc[-5] if len(
            ma7) >= 5 and ma7.iloc[-5] != 0 else 0
    ...
    # Collect training data
        for stock_code in list(stock_codes)[:50]:  # Limit untuk testing
            try:
        ...
        # Convert to numpy arrays with proper validation
            ...
            print(f"🔍 Raw targets sample: {all_targets[:10]}")
            print(f"🔍 Unique values in y: {np.unique(y)}")
            print(f"🔍 Y dtype: {y.dtype}")
            print(f"🔍 Y shape: {y.shape}")
            print(f"🔍 Y min/max: {y.min()}/{y.max()}")

         # Cek apakah ada nilai yang tidak valid
            invalid_mask = (y < 0) | (y > 2)
            if np.any(invalid_mask):
               print(f"❌ Invalid labels found at indices: {np.where(invalid_mask)[0]}")
               print(f"❌ Invalid values: {y[invalid_mask]}")
               return False

       #   Paksa konversi ke int yang benar
            y = y.astype(np.int32)
            y = np.clip(y, 0, 2)  # Pastikan range 0-2

      # Validate array shapes (baris yang sudah ada)
            if X.ndim != 2 or len(X) != len(y):
               logger.error("❌ Array shapes tidak valid")
               return False
            
            unique_labels = np.unique(y)
            valid_labels = {0, 1, 2}
            if not set(unique_labels).issubset(valid_labels):
               logger.error(f"❌ Invalid labels found: {unique_labels}. Expected: {valid_labels}")
               return False

         # Pastikan semua label ada
            if len(unique_labels) < 3:
              logger.warning(f"⚠️ Missing labels. Found: {unique_labels}. Expected: [0, 1, 2]")
    # Tambah dummy data untuk label yang hilang
            for missing_label in valid_labels - set(unique_labels):
                 X = np.vstack([X, X[0]])  # Duplikasi row pertama
                 y = np.append(y, missing_label)
        ...
        # Remove NaN/Inf values
            ...
            if len(X) < 10:
                logger.warning("❌ Data latih terlalu sedikit")
                return False
        ...
        # Print dataset info
            ...
            for label, count in zip(unique, counts):
                print(f"📈 {self.REVERSE_MAP[label]}: {count}")
    ...
    def prepare_features(self, data):
        ...
        for i in range(20, len(data) - 5):
            ...
            # Calculate target
                 ...
                 label_counter[target_label] += 1

            # Convert string label to numeric using consistent mapping
                 if target_label not in self.LABEL_MAP:
                    logger.warning(f"Invalid target_label: {target_label}, skipping")
                    continue

            # Pastikan mapping benar
                 numeric_label = self.LABEL_MAP[target_label]
                 if numeric_label not in [0, 1, 2]:
                   logger.error(f"Invalid numeric label: {numeric_label}")
                   continue
    ...
    def extract_pattern_features(self, pattern_data):
        ...
        try:
        # Pastikan pattern_data tidak kosong
        ...
        # MA patterns
            ma7_slope = (pattern_data['MA7'].iloc[-1] - pattern_data['MA7'].iloc[0]) / pattern_data['MA7'].iloc[0] if 'MA7' in pattern_data.columns and pattern_data['MA7'].iloc[0] != 0 else 0
            ma20_slope = (pattern_data['MA20'].iloc[-1] - pattern_data['MA20'].iloc[0]) / pattern_data['MA20'].iloc[0] if 'MA20' in pattern_data.columns and pattern_data['MA20'].iloc[0] != 0 else 0
        ...
        # CRITICAL: Validate and clean features
            validated_features = []
            for feature in features:
                if pd.isna(feature) or np.isnan(feature) or np.isinf(feature):
            ...
            logger.error(f"Error in extract_pattern_features: {str(e)}")
           # Return default feature vector dengan panjang yang sama
            return [0.0] * 17
    ...
    def calculate_optimal_entry(self, future_data):
        ...
        try:
            ...
            logger.debug(f"calculate_optimal_entry called with {len(future_data)} rows")
            ...
            for i in range(1, len(future_data)):
                ...
                # Logika yang lebih jelas
                    if max_profit > 0.05 and min_loss > -0.03:
                        return {'label': 'BUY'}
                    elif max_profit < -0.03 and min_loss < -0.05:
                        return {'label': 'SELL'}

            result = {'label': 'HOLD'}
            if result['label'] not in self.LABEL_MAP:
               logger.error(f"Invalid label generated: {result['label']}")
               result['label'] = 'HOLD'
            return result
        
        except Exception as e:
            logger.error(f"Error in calculate_optimal_entry: {str(e)}")
            logger.debug(f"Returning label: HOLD")
            return {'label': 'HOLD'}
    ...
    # Check cache
        ...
        # Get features
            ...
            if feature_vector is None or len(feature_vector) != 17:
                logger.error("Feature vector invalid")
                return self.generate_basic_signals(data)
        ...
        # Pastikan prediction adalah integer valid
            if prediction not in self.REVERSE_MAP:
               logger.warning(f"Invalid prediction: {prediction}, using HOLD")
               prediction = 1  # Default ke HOLD
            signal_type = self.REVERSE_MAP[prediction]
        
        # Generate signals
            current_price = data['Close'].iloc[-1]
            signals = self.generate_signals_by_type(signal_type, current_price)
        ...
        # Cache results
            ...
            logger.error(f"Error in predict_signals: {str(e)}")
            return self.generate_basic_signals(data)
    ...
    def generate_dynamic_signals_with_entry(self, entry_price, confidence,
        ...
        signal_type = "BUY" if confidence >= 0.75 else "SELL"
        ...
        if signal_type == "BUY":
            return {
                ...
                'target_estimate': predicted_high * 1.09
            }
        else:
            return {
                'signal': 'SELL',
                'entry': entry_price,
                'profit_1': predicted_low * 0.98,
                'profit_2': predicted_low * 0.96,
                'profit_3': predicted_low * 0.93,
                'stop_loss_1': predicted_high * 1.02,
                'stop_loss_2': predicted_high,
                'stop_loss_3': predicted_high * 1.04,
                'target_estimate': predicted_low * 0.9
            }

    def generate_dynamic_sell_signals(self,
                                      current_price,
                                      data=None,
                                      dynamic_levels=None):
...
⚠️ Penting: Lakukan analisa sendiri sebelum memutuskan! Sinyal bot hanya referensi. Bot TIDAK bertanggung jawab atas kerugian apa pun.
...
async def clear_cache_command(update: Update,
