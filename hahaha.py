...
class StockAnalyzer:
    ...
    # Train new model
        ...
        try:
            X = np.asarray(all_features, "if_needed")
            y = np.asarray(all_targets, "if_needed")
            ...
            y = np.asarray([label_map[label] for label in y], "if_needed")
    ...
    def predict_signals(self, data, stock_code):
        ...
        try:
            ...
            # Predict
            X = np.asarray([feature_vector], "if_needed")
    ...
    def predict_optimal_entry(self, data):
        ...
        # Predict signal and confidence
            X = np.asarray([features], "if_needed")
            X_scaled = self.scaler.transform(X)
            prediction = self.model.predict(X_scaled)[0]
