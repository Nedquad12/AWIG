...
class StockAnalyzer:
    ...
    # Train new model
        ...
        try:
            X = np.asarray(all_features, copy=True)
            y = np.asarray(all_targets, copy=True)
            ...
            y = np.asarray([label_map[label] for label in y], copy=True)
    ...
    def predict_signals(self, data, stock_code):
        ...
        try:
            ...
            # Predict
            X = np.asarray([feature_vector], copy=True)
    ...
    def predict_optimal_entry(self, data):
        ...
        # Predict signal and confidence
            X = np.asarray([features], copy=True)
            X_scaled = self.scaler.transform(X)
            prediction = self.model.predict(X_scaled)[0]
