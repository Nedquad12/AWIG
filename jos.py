...
class StockAnalyzer:
    ...
    # Train new model
        ...
        try:
            ...
            y = np.asarray(all_targets, dtype=np.int32)
    ...
    def create_chart(self, data, stock_code, signals):
        ...
        # Save to bytes
        buf = io.BytesIO()
        fig.text(0.5, 0.02, '@saberial_link', fontsize=14,
        color='gray', ha='center', va='center', alpha=0.3)
...
allowed_group_id =-1002849988768
 # c def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
     """Start command handler"""
     
     # 🚨 Cek jika chat ini private (DM ke bot)
     if update.effective_chat.type == "private":
         await update.message.reply_text(
            "Hai! Ingin gunakan bot ini secara pribadi atau ditambah ke grup yang diinginkan? Harap hubungi: Telgram @Rendanggedang atau twitter https://x.com/saberial_link/.",
            parse_mode="MarkdownV2"
        )
         return
     # 🚨 Tolak jika bukan grup resmi
...
def main():
    ...
    if not BOT_TOKEN:
       raise Exception("❌ BOT_TOKEN belum diset di environment variable!")
